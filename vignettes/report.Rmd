...
data.list <- list(
  ...
  prostate=list(
    ...
    feature.mat=as.matrix(ElemStatLearn::prostate[, 1:(ncol(ElemStatLearn::prostate)-2)])),
  ozone=list(
    label.vec=ElemStatLearn::ozone$ozone,
    feature.mat=as.matrix(ElemStatLearn::ozone[, 2:ncol(ElemStatLearn::ozone)]))
)
...
for(data.name in names(data.list))
{
  ...
  for(test.fold in 1:n.folds)
  {
    ...
    is.train <- !is.test
    X.train <- one.data.set$features[is.train,]
    y.train <- one.data.set$labels[is.train]
    ...
    {  
       ...
       resultES <- LMSquareLossEarlyStoppingCV(X.train,
                                              y.train,
                                              fold.vec,
                                              max.iterations)
       resultREG <- LMSquareLossIterations(X.train,
                                       y.train,
                                       fold.vec,
                                       max.iterations)
...
for(data.name in names(data.list))
{
  ...
  #Regression: the mean of the training labels/outputs.
    ...
    prdictionBase <- mean(y.vec)
    baseline.append(mean(y.vec))
    
    
    # print(data.name)
    # print(data.list[[data.name]])
    
    resultES <- LMSquareLossEarlyStoppingCV(X.mat,
                                            y.vec,
                                            fold.vec,
                                            max.iterations)
    result <- LMSquareLossIterations(X.mat,
                                     y.vec,
                                     fold.vec,
                                     max.iterations)
  ...
  mat <- cbind(resultES$penalty.vec, result$penalty.vec, prdictionBase)
