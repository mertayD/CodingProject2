# write tests for that R function, in tests/testthat/test-LMLogisticLossL2CV:
...
library(testthat)
context("test-LMLogisticLossL2CV")


test_that("LMLogisticLossL2CV computes the right demensions", {
...
test_that("LMLogisticLossL2CV throws errors", {
  data(SAheart , package = "ElemStatLearn")
  X.mat<-SAheart [1:50,-9]
  y.vec<-SAheart [1:50, 9]
  max.iterations <- 100
  ...
  expect_error(LMLogisticLossL2CV(X.mat, y.vec, fold.vec, max.iterations), "Feature matrix or Label vec has unexpected dimensions")
